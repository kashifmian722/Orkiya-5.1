...
public function searchProducts(Request $request, MerchantEntity $merchant): JsonResponse
{
  ...
  /**
 * @Route("/merchant/{id}/likes", name="storefront.merchant.likes", defaults={"csrf_protected"=false})
 */
public function likes(Request $request, string $id, SalesChannelContext $context)
{
    $customer = $context->getCustomer();
    if ($customer) {
        $criteria = new Criteria([$id]);
        $merchant = $this->merchantRepository->search($criteria, $context->getContext())->first();
        $likes = 0;
        if (isset($merchant->getCustomFields()['likes'])) {
            $likes = (int) $merchant->getCustomFields()['likes'];
        }
        $this->merchantRepository->update([
            [
                'id' => $id,
